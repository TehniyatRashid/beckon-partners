...
import { calculateDiagnosticScore } from '../utils/assessmentScoring';
...
export const AssessmentPage: React.FC<AssessmentPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      budgetRange,
      additionalNotes,
      // Backward compatibility for current Google Sheet Apps Script:
      healthLabel: diagnosticResult.healthTier,
      healthDetail: diagnosticResult.healthDetail,
      recoverableTime: diagnosticResult.recoverableRange,
      // New diagnostic keys for future expansion:
      healthScore: diagnosticResult.healthScore,
      healthTier: diagnosticResult.healthTier,
      scoreDeductions: diagnosticResult.deductions,
      estimatedRecoverableAdminCapacity: recoverableTime,
      recoverableHoursCalculated: Number(diagnosticResult.recoverableHours.toFixed(1))
    };
  ...
  const diagnosticResult = calculateDiagnosticScore({
    teamSize,
    primaryFriction: effectiveFriction,
    selectedTools,
    systemIntegrationStatus,
  ...
  const recoverableTime = diagnosticResult.recoverableRange;
          ...
          {!isSubmitted ? (
                          ...
                          {revenueOptions.map((rev, idx) => (
                            <div
                              ...
                              className={`p-3 sm:p-3.5 rounded-[10px] border transition-all cursor-pointer flex items-center justify-between text-xs sm:text-[13px] ${
                                revenueRange === rev
                                  ? 'bg-white/12 border-[#C7A472] text-white font-semibold ring-1 ring-[#C7A472]/40'
                                  : 'bg-white/[0.03] border-white/10 text-[#C4C7D0] hover:bg-white/[0.06] hover:border-[#FAF8F5]/20'
                              }`}
                            >
                              <span>{rev}</span>
                    ...
                    {currentStep === 9 && (
                      <div className="space-y-3 sm:space-y-3.5">
                        <div>
                          ...
                          <h2 className="text-xl sm:text-2xl lg:text-[28px] font-bold text-white tracking-tight leading-snug font-inter">
                            What level of investment would be realistic?
                          </h2>
                        </div>
                      ...
                      <div className="space-y-3 sm:space-y-3.5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                          <div className="p-3 rounded-[10px] bg-white/5 border border-white/10 space-y-0.5">
                            <div className="text-[9px] text-[#8C8F98] uppercase font-mono">Business System Health</div>
                            <div className="text-sm sm:text-base font-bold text-[#C7A472] leading-tight">
                              {diagnosticResult.healthScore}/100
                            </div>
                            <div className="text-[10px] text-[#C4C7D0]">{diagnosticResult.healthTier}</div>
                          </div>

                          <div className="p-3 rounded-[10px] bg-white/5 border border-white/10 space-y-0.5">
                            <div className="text-[9px] text-[#8C8F98] uppercase font-mono">Estimated Recoverable Admin Capacity</div>
                            <div className="text-sm sm:text-base font-bold text-white leading-tight">
                              {recoverableTime}
                            </div>
                            <div className="text-[10px] text-[#C4C7D0]">Preliminary diagnostic estimate</div>
                          </div>
                        ...
                        <p className="text-[9px] text-[#8C8F98] leading-relaxed">
                          *Preliminary estimate based on diagnostic responses. Actual opportunity is validated during discovery.
                        </p>

                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-center">
                          {[
                            ['FASTER RESPONSE', 'Reduce missed opportunities'],
                            ['LESS MANUAL WORK', 'Reclaim operational capacity'],
                            ['BETTER MARGINS', 'Improve profitability visibility'],
                          ].map(([title, detail]) => (
                            <div key={title} className="p-2.5 rounded-[10px] bg-white/[0.03] border border-white/10">
                              <div className="text-[9px] font-mono font-semibold text-[#C7A472]">{title}</div>
                              <div className="text-[9px] text-[#C4C7D0] mt-0.5">{detail}</div>
                            </div>
                          ))}
                        </div>
            ...
            <div className="w-full max-w-xl mx-auto my-auto py-8 text-center space-y-6 relative z-10">
              ...
              <div className="p-5 rounded-[18px] bg-white/5 border border-white/10 text-left font-sans text-xs space-y-2.5">
                ...
                <div className="flex justify-between border-b border-white/10 pb-2">
                  <span className="text-[#8C8F98]">Business System Health:</span>
                  <span className="text-[#C7A472] font-semibold">{diagnosticResult.healthScore}/100 · {diagnosticResult.healthTier}</span>
                </div>
                <div className="flex justify-between border-b border-white/10 pb-2">
                  <span className="text-[#8C8F98]">Est. Recoverable Hours:</span>
                  <span className="text-white font-medium">{recoverableTime}</span>
                </div>
                <p className="text-[9px] text-[#8C8F98] leading-relaxed pt-1">
                  *Preliminary estimate based on diagnostic responses. Actual opportunity is validated during discovery.
                </p>
